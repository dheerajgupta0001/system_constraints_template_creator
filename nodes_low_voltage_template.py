...
try:
    ...
    sql_fetch ="""select * from nodes_low_voltage_data \
        where start_date IN (select start_date as s from nodes_low_voltage_data where rownum<2)"""
...
for i in df.index:
    ...
    lowVoltageNodeData.append(tempDict)
    #print(tempDict)
    
print(len(lowVoltageNodeData))